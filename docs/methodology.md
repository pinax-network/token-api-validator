...
**Definition**: Of the fields where both our API and the reference provider returned non-null values, what percentage matched within tolerance?

```
Accuracy = matches / (matches + mismatches)
```

Null comparisons (where either side returned no data) are excluded from accuracy.
...
- It is **excluded** from the accuracy metric (not counted as match or mismatch)
- It **is counted** in the coverage metric (as a "non-comparable" comparison)
...
Null comparisons (where either side returned no data) are excluded from regression tracking on both sides.
