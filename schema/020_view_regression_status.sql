-- Regression classification for every comparable comparison across all domains.
...
SELECT
    run_at, domain, network, contract, symbol, field, entity, provider,
...
WHERE is_comparable
WINDOW w AS (
    PARTITION BY domain, network, contract, field, entity, provider
    ORDER BY run_at
