...
- Solana RPC provider uses dRPC (`lb.drpc.org/ogrpc?network=solana`) with `Drpc-Key` header auth. Metadata: `getTokenSupply` for supply/decimals, Metaplex Token Metadata PDA lookup via `getAccountInfo` for name/symbol. Balances: `getTokenAccountBalance` per holder (token account addresses from Token API), batched via JSON-RPC. PDA derivation uses `@noble/hashes/sha256` and `@noble/curves/ed25519` (transitive deps from viem, no new dependency). Balance values are raw integer strings (no scaling needed — matches Token API format).
