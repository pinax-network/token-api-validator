...
- Name/symbol comparisons are provider-filtered to avoid comparing curated names against on-chain values. Override status is determined per token by comparing `tokens.json` curated name against the RPC result. Override tokens (curated ≠ on-chain) are compared against non-RPC providers only; non-override tokens are compared against RPC only. `RPC_PROVIDERS` in `src/validator.ts` defines which providers are on-chain (`rpc`, `solana-rpc`). See `docs/methodology.md` for rationale.
